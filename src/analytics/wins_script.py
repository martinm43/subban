...
def get_wins(team_id, season_year, start_datetime, end_datetime):
    ...
    visitor_OTW_total = sum([1 if x[2] != "Regulation" and x[0] > x[1] else 0 for x in visitor_results])
    ...
    home_OTW_total = sum([1 if x[2] != "Regulation" and x[0] > x[1] else 0 for x in home_results])
    home_OTL_total = sum([1 if x[2] != "Regulation" and x[1] < x[0] else 0 for x in home_results])
    
    home_games_total = len(home_results)

    visitor_record = str(visitor_wins_total+visitor_OTW_total) + "-" + str(visitor_games_total - visitor_OTW_total - visitor_OTL_total - visitor_wins_total) + "-" + str(visitor_OTL_total)
    home_record = str(home_wins_total+home_OTW_total) + "-" + str(home_games_total - home_OTW_total - home_OTL_total - home_wins_total) + "-" + str(home_OTL_total)

    record = (
        str(visitor_wins_total + home_wins_total)
        + "-"
        + str(visitor_games_total + home_games_total - visitor_OTW_total - home_OTW_total - visitor_wins_total - home_wins_total)
        + "-" 
        + str(home_OTL_total+visitor_OTL_total)
    )
    pts =  2*(visitor_wins_total+home_wins_total+visitor_OTW_total+home_OTW_total)+1*(visitor_OTL_total+home_OTL_total)
    try:
